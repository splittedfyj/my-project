...
/* Layout */
import Layout from '@/layout' // 在路由之中引入layout组件。引入layout下的index.vue
 ...
 */
// 对外暴露了一个常量，常量路由，里面放的数组，里面是路由的信息
...
// 对外暴露了一个箭头函数，箭头函数一执行，会返回一个路由器对象
const createRouter = () => new Router({
  // mode: 'history', // require service support
  scrollBehavior: () => ({ y: 0 }), // 每次进行路由切换时，页面都会自动滚动到顶部
  routes: constantRoutes // 把所有的路由进行注册   把routes的所有信息放在了constantRoutes里面了，放在数组里包这
}) // 外层放了一个常量，还是createRouter函数一执行，才会返回Router实例的一个对象，滚动的时候y=0,以及配置了相关路由
